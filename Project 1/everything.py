...
#Finding the amount of labels (numbers) in a given image
...
#Drawing rectangles around the numbers
def drawRec(im, bim, counter, list, image):
    width, height = 4, counter
    rectangles = [[0 for x in range(width)] for y in range(height)] #will be used to store minx, miny, maxx, maxy values of numbers
    ...
    for i in range(nrow):
        for j in range(ncol):
            if list.__contains__(im[i][j]):
                ...
                if rectangles[index][0] == 0 and rectangles[index][1] == 0 and rectangles[index][2] == 0 and rectangles[index][3] == 0: #if this is the first time w/ the label
...
def featureVectors(bim, rectangles, counter):
    ...
    for k in range(counter):
        ...
        for i in range(len(arrayOfResizedImg)):
            featuresHu.append(calcMomentsHu(arrayOfResizedImg[i]))
...
def saveFile(image):
    moments = findAllFeatureVectors(image) # calculating the moments of a given image
...
def comparisonForOne(inputMom, trainMom):
    ...
    for m in range(len(allDistances)):
        matches.append(findMin(allDistances[m]))
...
def comparisonForAll(image, fileNames): #takes an image to process and an array of files with train image's moments
    ...
    for i in fileNames:
        ...
        matchesForOne = comparisonForOne(inputMom, realMom) #all character's match to one font (one train image)
    ...
    for j in range(len(transposed)):
        finalMatches.append(findMostFrequent(transposed[j]))
...
def findAllFeatureVectors(input_img):
    ...
    a_bin = threshold(a, 150, ONE, 0)
    im = Image.fromarray(a_bin)  # from np array to PIL format
    a_bin = np.asarray(im)

    image_name = input_img
    imO = label_8_connected(a_bin, ONE)
    counterO, listO = findLabelAmount(a_bin, imO)
    rectanglesO = drawRec(imO, a_bin, counterO, listO, image_name)
    featuresHuO = featureVectors(a_bin, rectanglesO, counterO)
...
def matchNumbers(input_img, fileNames):
    ...
    a_bin = threshold(a, 150, ONE, 0)
    im = Image.fromarray(a_bin)  # from np array to PIL format
    a_bin = np.asarray(im)  # (ME) to color the original image
    ...
    rectanglesO = drawRec(imO, a_bin, counterO, listO, image_name)
    featuresHuO = featureVectors(a_bin, rectanglesO, counterO)
    finalMatchesO = comparisonForAll(input_img, fileNames)
    addText('output.png', finalMatchesO, rectanglesO)
...
def FindNumbersClicked(event):
    ...
    messagebox.showinfo(" ", "Please wait for program to finish. This might take a while.")
    matchNumbers(img, fileNames)
...
FindNumbersButton = tk.Button(root, text="Find Numbers", highlightbackground='#3E4149')
